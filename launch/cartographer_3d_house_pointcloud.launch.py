...
def generate_launch_description():
    ...
    default_pbstream = os.path.join(pkg_root, 'maps', 'agv_house_cartographer_3d_pointcloud.pbstream')
    default_accumulated_state = os.path.join(pkg_root, 'maps', 'agv_house_cartographer_3d_pointcloud_accumulated.pkl')
    ...
    load_accumulated_state_arg = DeclareLaunchArgument(
        ...
        default_value='false',
        description='Load the previous accumulated RViz point cloud. Keep false for a clean SLAM run.',
    )
    ...
    save_map_hint = LogInfo(
        msg=[
            '3D house mapping: RViz se bat san layer "Accumulated Map" de giu lai nhung noi robot da quet qua trong cung mot lan chay. ',
            'Cartographer van luu map chinh qua /write_state sang pbstream. ',
            'State cloud tich luy duoc luu/nap tai: ',
            LaunchConfiguration('accumulated_state_path'),
            '. ',
            'Luu map chinh dang pbstream: export ROS_DOMAIN_ID=69 && source ~/ros2_ws/install/setup.bash && ',
