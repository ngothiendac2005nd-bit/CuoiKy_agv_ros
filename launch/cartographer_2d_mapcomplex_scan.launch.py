...
def generate_launch_description():
    ...
    default_map_base = os.path.join(pkg_root, 'maps', 'agv_mapcomplex_scan_map')
    ...
    save_map_hint = LogInfo(
        msg=[
            'Map complex 2D: quet xong thi mo terminal moi va chay ',
            'export ROS_DOMAIN_ID=69 && ',
            'source ~/ros2_ws/install/setup.bash && ',
            'ros2 run nav2_map_server map_saver_cli -f ',
            LaunchConfiguration('map_basename'),
            '. Spawn mac dinh dang la x=-7.2, y=-5.6, yaw=0.0.',
        ]
    )
