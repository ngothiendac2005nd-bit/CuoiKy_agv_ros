...
def generate_launch_description():
    ...
    default_map = os.path.join(pkg_root, 'maps', 'agv_mapcomplex_scan_map.yaml')
    source_world_file = os.path.join(pkg_root, 'worlds', 'mapcomplex_scan.world')
    installed_world_file = os.path.join(pkg_share, 'worlds', 'mapcomplex_scan.world')
    ...
    x_pose_arg = DeclareLaunchArgument('x_pose', default_value='-7.2')
    y_pose_arg = DeclareLaunchArgument('y_pose', default_value='-5.6')
    ...
    map_arg = DeclareLaunchArgument(
        ...
        description='Absolute path to the saved mapcomplex_scan map yaml file'
    )
    ...
    initial_pose_hint = LogInfo(
        msg='Map complex nav: dung map da luu agv_mapcomplex_scan_map.yaml. Neu scan khong khop map, dat lai 2D Pose Estimate hoac truyen initial_pose_x/y/yaw.'
    )
